...
class RAGSystem:
        ...
        try:
            ...
            for sub_q in subqueries:
                ...
                docs = self.vector_store.hybrid_search_with_rerank(
                    sub_q, k=conf.CANDIDATE_M//2 # 使用 K
                )
    ...
    def generate_answer(self, query):
        ...
        if query_category == "通用知识":
            # 通用知识：直接调用大模型回答，不进行 RAG 检索
            logger.info("通用知识问题，直接调用大模型回答")
            try:
            ...
            # 专业咨询：执行完整的 RAG 检索流程
            logger.info("专业咨询问题，执行 RAG 检索流程")
            #   选择检索策略
            strategy = self.strategy_selector.select_strategy(query)
            #  检索相关文档
            context_docs = self.retrieve_and_merge(query, strategy=strategy)  # 传递 strategy
...
if __name__ == '__main__':
    ...
    answer = rag_system.generate_answer(query="AI 学科的课程大纲内容有什么")
    print(answer)
    # rag_system._retrieve_with_subqueries(query="AI 和 JAVA 的区别是什么？")
    # result = rag_system._retrieve_with_hyde(query="AI 课程的 NLP 的技术有哪些?")
    # print(result)
    # print(len(result))