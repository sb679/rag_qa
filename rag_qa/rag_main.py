...
def main(query_mode=True, directory_path="data"):
    ...
    if not query_mode:
        ...
        for source_dir in conf.VALID_SOURCES:
            # 尝试两种可能的目录格式：带 _data 后缀和不带后缀
            dir_path_with_suffix = os.path.join(directory_path, f"{source_dir}_data")
            dir_path_without_suffix = os.path.join(directory_path, source_dir)
                
            # 优先使用带 _data 后缀的目录，如果不存在则尝试不带后缀的
            if os.path.exists(dir_path_with_suffix):
                dir_path = dir_path_with_suffix
            elif os.path.exists(dir_path_without_suffix):
                dir_path = dir_path_without_suffix
            else:
                logger.warning(f"目录 {dir_path_with_suffix} 和 {dir_path_without_suffix} 都不存在，跳过处理")
                continue
